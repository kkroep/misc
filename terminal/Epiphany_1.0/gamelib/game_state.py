...
class GameState:
    ...
    def __parse_state(self, state_line):
        ...
        #gamelib.debug_write('player 2 units {} = turn {}'.format(p2units, self.turn_number))

        self.__create_parsed_units(p1units, 0)
        self.__create_parsed_units(p2units, 1)

    def __parse_action_state(self, state_line):
        state = json.loads(state_line)
    ...
    def find_path_to_point(self, start_location, target_location):
        """Gets the path a unit at a given location would take
